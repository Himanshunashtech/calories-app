...
export default function RecipeDetailPage() {
  ...
  const handleLogMeal = () => {
    if (recipe) {
      addMealLog({
        // id will be generated by addMealLog
        // date will be set to now by addMealLog
        category: 'Snack', // Default or let user choose
        description: recipe.name,
        calories: recipe.calories || 0, // Ensure these exist on your recipe type
        protein: recipe.protein || 0,
        carbs: recipe.carbs || 0,
        fat: recipe.fat || 0,
        nutritionalInfo: recipe.description, // Or more detailed info
        photoDataUri: recipe.image.startsWith('https://placehold.co') ? undefined : recipe.image, // Avoid logging placeholders
        // detailedNutrients: recipe.detailedNutrients, // If available
        carbonFootprintEstimate: recipe.ecoScore === 'A' ? 0.5 : recipe.ecoScore === 'B' ? 1.0 : 1.5 // Rough estimate
      });
      toast({
        title: `Meal Logged!`,
      ...
      <Card className="shadow-xl overflow-hidden">
        ...
        <CardFooter className="p-4 md:p-6 border-t flex flex-col sm:flex-row gap-2">
          <Button size="lg" className="w-full sm:flex-1" onClick={handleLogMeal}>
           ...
           <Button size="lg" variant="outline" className="w-full sm:flex-1" disabled>
